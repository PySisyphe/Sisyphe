...
def parcellation(voxel, pnet, device, mng=None):
    """
    ...
    # ------------------------
    if mng is not None: mng['msg'] = 'Parcellation, coronal inference...'
    out_c = parcellate(coronal, pnet, device, "Coronal", mng=mng).permute(1, 3, 0, 2)
    torch.cuda.empty_cache()

    # ------------------------
    # Sagittal view inference
    # ------------------------
    if mng is not None: mng['msg'] = 'Parcellation, sagittal inference...'
    out_s = parcellate(sagittal, pnet, device, "Sagittal", mng=mng).permute(1, 0, 2, 3)
    torch.cuda.empty_cache()
    ...
    # ------------------------
    if mng is not None: mng['msg'] = 'Parcellation, axial inference...'
    out_a = parcellate(axial, pnet, device, "Axial", mng=mng).permute(1, 3, 2, 0)
    torch.cuda.empty_cache()
    if mng is not None: mng['max'] = 0
    if mng is not None: mng['msg'] = 'Parcellation...'
