...
def separate(voxel, model, device, mng=None):
    """
...
def hemisphere(voxel, hnet, device, mng=None):
    """
    ...
    if mng is not None: mng['max'] = 448 # 224 * 2
    if mng is not None: mng['msg'] = 'Hemisphere separation, coronal inference...'
    out_c = separate(coronal, hnet, device, mng).permute(1, 3, 0, 2)
    if mng is not None: mng['msg'] = 'Hemisphere separation, axial inference...'
    out_a = separate(transverse, hnet, device, mng).permute(1, 3, 2, 0)
    if mng is not None: mng['max'] = 0
    if mng is not None: mng['msg'] = 'Hemisphere separation...'
