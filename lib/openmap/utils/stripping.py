...
from .functions import normalize, reimburse_conform


def strip(voxel, model, device, mng=None):
    """
    ...
    Args:
        ...
        mng (DictProxy | None): used for progress bar.
    ...
    with torch.inference_mode():
        ...
        for i in range(1, 225):
            ...
            box[i - 1] = x_out
            if mng is not None: mng['inc'] = 1

        # Return as a 3D mask tensor
        return box.reshape(224, 224, 224)


def stripping(output_dir, basename, voxel, odata, data, ssnet, shift, device, mng=None):
    """
    ...
    if mng is not None: mng['max'] = 672 # 224 * 3
    if mng is not None: mng['msg'] = 'Stripping, coronal inference...'
    out_c = strip(coronal, ssnet, device, mng).permute(2, 0, 1)  # coronal → native orientation
    if mng is not None: mng['msg'] = 'Stripping, sagittal inference...'
    out_s = strip(sagittal, ssnet, device, mng)  # sagittal
    if mng is not None: mng['msg'] = 'Stripping, axial inference...'
    out_a = strip(axial, ssnet, device, mng).permute(2, 1, 0)  # axial → native orientation
    if mng is not None: mng['max'] = 0
    if mng is not None: mng['msg'] = 'Stripping...'
