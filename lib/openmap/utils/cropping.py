...
def crop(voxel, model, device, mng=None):
    """
...
def cropping(output_dir, basename, odata, data, cnet, device, mng=None):
    """
    ...
    if mng is not None: mng['max'] = 512
    if mng is not None: mng['msg'] = 'Cropping, coronal inference...'
    out_c = crop(coronal, cnet, device, mng).permute(2, 0, 1)
    if mng is not None: mng['msg'] = 'Cropping, sagittal inference...'
    out_s = crop(sagittal, cnet, device, mng)
    if mng is not None: mng['max'] = 0
    if mng is not None: mng['msg'] = 'Cropping...'
