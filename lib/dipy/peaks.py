...
def peaks_from_model(
    ...
    *,
    ...
    wait : multiprocessing.managers.DictProxy | None
        optional multiprocessing shared dict (DictProxy)
    ...
    if wait is not None:
        c = 0
        cmax = int(np.array(shape).cumprod()[-1]) - 1
        wait['max'] = 100
    # Revision 27/07/2026 >
