    ...
    *,
    ...
    for k in range(patch_radius_arr[2], arr.shape[2] - patch_radius_arr[2]):
        ...
        if wait is not None and k > patch_radius_arr[2]:
            ...
            if m == 0:wait.addInformationText('Estimated time remaining {} s.'.format(s))
            else: wait.addInformationText('Estimated time remaining {} min {} s.'.format(m, s))
        # Revision 18/06/2024 from original dipy >
        ...
        # < Revision 18/06/2024 from original dipy
        if wait is not None:
            wait.setCurrentProgressValue(int(k - patch_radius_arr[2] + 1))
        # Revision 18/06/2024 from original dipy >
    # < Revision 18/06/2024 from original dipy
    if wait is not None:
