...
from numpy import *
from Sisyphe.lib.tgvqsm.qsm_tgv_cython_helper import *
from Sisyphe.gui.dialogWait import DialogWait
from Sisyphe.gui.dialogWait import UserAbortException

import Sisyphe.lib.tgvqsm.progressbar as progressbar

from time import time


def qsm_tgv(laplace_phi0,
            mask,
            res,
            alpha=(0.2, 0.1),
            iterations=1000,
    ...
    if wait is not None:
        wait.setProgressRange(0, iterations)
    ...
    while k < iterations:
        ...
        if wait is not None:
            if wait.getStopped(): raise UserAbortException
        ...
        if wait is not None:
            if wait.getStopped(): raise UserAbortException
            wait.setCurrentProgressValue(k)
            if k > 10:
                r = iterations - k
                r = ((time() - start) / k) * r
                if r <= 60.0: wait.addInformationText('Estimated time remaining {} sec.'.format(int(r)))
                else: wait.addInformationText('Estimated time remaining {} min.'.format(int(r / 60)))
