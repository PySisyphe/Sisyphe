...
def dwiPreprocessing(vols: SisypheVolumeCollection,
                     ...
                     save: bool = False,
                     wait: DialogWait | DictProxy | None = None) -> tuple[SisypheVolumeCollection, SisypheROI | None] | None:
    """
    Parameters
    ----------
    ...
    if brainseg is not None:
        ...
        if wait is not None:
            if isinstance(wait, DialogWait): wait.setInformationText('Mask processing...')
            elif isinstance(wait, DictProxy): wait['msg'] = 'Mask processing...'
        """
        ...
        if save:
            ...
            if wait is not None:
                if isinstance(wait, DialogWait): wait.setInformationText('Save {}...'.format(basename(rmask.getFilename())))
                elif isinstance(wait, DictProxy): wait['msg'] = 'Save {}...'.format(basename(rmask.getFilename()))
            rmask.save()
        else: rmask = None
    # Gibbs suppression
    if gibbs is not None:
        ...
        if wait is not None:
            if isinstance(wait, DialogWait): wait.setInformationText('Gibbs correction...')
        ...
        for i in range(n):
            ...
            #    return None
        if isinstance(wait, DialogWait): wait.setProgressVisibility(False)
    ...
    if denoise is not None:
        ...
        if denoise['algo'] == _DENOISE[0]:
            ...
            if wait is not None:
                if isinstance(wait, DialogWait): wait.setInformationText('Noise estimation...')
                elif isinstance(wait, DictProxy): wait['msg'] = 'Noise estimation...'
        ...
        # Adaptive soft coefficient matching denoising
            ...
            sigma = dwiNoiseEstimation(vols, noisealgo, rec, ncoils, nphase, wait)
            if wait is not None:
                if isinstance(wait, DialogWait): wait.setInformationText('Adaptive soft coefficient matching denoising...')
            ...
            """
            if wait is not None:
                if isinstance(wait, DialogWait): wait.setInformationText('Stage 1/3 - non-local means, small patch...')
                elif isinstance(wait, DictProxy): wait['msg'] = 'Stage 1/3 - non-local means, small patch...'
            img_small = non_local_means(imgs, sigma=sigma, mask=mask, patch_radius=1, block_radius=1, rician=True, wait=wait)
            if wait is not None:
                if isinstance(wait, DialogWait): wait.setInformationText('Stage 2/3 - non-local means, large patch...')
