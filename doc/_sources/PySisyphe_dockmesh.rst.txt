...
|save|
	Left-click to save mesh (PySisyphe mesh file, .xmesh), mesh name is used as file name.
...
|open|
	Left-click to show mesh selection dialog (PySisyphe mesh file, .xmesh), adds selected mesh(es) to the mesh list and displays them in the orthogonal view widget. **This mesh must be in the same space as the reference volume displayed in the view widget i.e. same space ID**.
...
|console|
	Left-click to copy checked Mesh(es) to the PySisyphe console as a list of SisypheMesh instances (variable name 'meshes').

|view|
	Left-click to show/hide all mesh(es) of the list.
...
|labels|
	Left-click to label checked Mesh(es) with labels from an atlas. A dialog box is displayed in which you can select one or more atlases. The results are similar to those described in :ref:`menu-section-mesh-labeling`.
...
|cubefeature|
	Left-click to calculate checked mesh(es) geometric features (volume, surface, bounds, center of mass coordinates, major axis vector, mid-axis vector, minor axis vector, sizes in each vector directions) and displays them in a dialog box. Features can be stored in various formats (CSV, JSON, latex TEX, TEXT, Word XLSX, PySisyphe XSHEET). They can be copied to the PySisyphe console as a numpy ndarray variable named 'df'.
