...
|tracking|
	...
	The list box on the left (1) shows avalaible ROI(s) i.e. ROI(s) loaded in :ref:`ROI dock <page-dockroi>`. It is possible to load ROI(s) from disk into this box, provided they have the same space ID or, failing that, the same field of view. Left click on **Add ROI(s)** to display a ROI selection dialog. There are two list boxes on the right, the top box lists the inclusion ROI(s) and the bottom box lists the exclusion ROI(s). The |left| buttons moves a selected ROI from the box of available ROI(s) to the box of inclusion or exclusion. The |right| buttons removes a selected ROI from the include or exclude box and returns it to the box of available ROI(s).
...
|open|
	Left-click to show a Bundle selection dialog (PySisyphe tractogram file, .xtracts), adds selected Bundle(s) to the list and displays them in the view widget. **This (These) Bundle(s) must be in the same space as the reference volume displayed in the view widget i.e. same space ID**.
...
|view|
	Left-click to show/hide all Bundle(s) of the list.
	
|cross|
	Left-click to remove checked Bundle(s) from list and from view widget(s).

|console|
	Left-click to copy checked Bundle(s) to the PySisyphe console as a list of SisypheStreamlines instances (variable name 'bundles').
...
|statistics|
	...
	- **Save bitmap**, show a dialog box to save a bitmap of the displayed chart (supported bitmap formats BMP, JPG, PNG, TIFF and SVG)
