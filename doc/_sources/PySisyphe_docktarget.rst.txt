...
|targets|
	Left-click to show a dialog box to add equidistant targets along the selected trajectory. The targets are added based on the target extremity of the selected trajectory.

	- Use the **Target Count** spinbox to set the number of targets to create (1 to 30).
	- Use the **First target spacing from extremity** spinbox to set the distance of the first target from the target extremity of the selected trajectory (-10 to +10 mm) .
	- Use the **Spacing Between Targets** spinbox to set the distance between targets (1.0 to 10.0 mm).
	- Toggle the **Target Properties** button to edit the display settings that will be applied to all targets (see :ref:`target settings details <target-section-settings>`).

	A list of electrodes is displayed above with the following fields: *manufacturer*, *model* code, contact *size* in mm, contact *spacing* in mm and number of *contacts*. Double-click on an electrode to fill in the **First target spacing from extremity** and **First target spacing from extremity** fields with this electrode geometry.

	Click **OK** to add targets in the view widget.

|save|
	Left-click show a dialog box to save all Target(s)/Trajectory(ies) of the list in a PySisyphe tools file (.xtools).

|delete|
	Left-click to remove all Target(s)/Trajectory(ies) from the list and from the view widget.

|console|
	Left-click to copy checked Tool(s) to the PySisyphe console as a list of HandleWidget/LineWidget instances (variable name 'tools').

|lock|
	Left-click to lock/unlock all Target(s)/Trajectory(ies) of the list.

|view|
	Left-click to show/hide all Target(s)/Trajectory(ies) of the list.

.. _target-section-settings:

|settings|
	Left-click to show a dialog to edit Target/Trajectory properties:

	The displayed settings are those of the selected tool.
	...
	- **Safety radius**, to define margin displayed as a sphere around Target in mm, margin is displayed as cylinder around Trajectory in mm.
	- **safety visbility**, checkbox to select safety margin visibility.
	- **Tolerance**, to define tolerance around handles (in pixels) for mouse selection.

	The changes are applied to the checked tools. If no tool is checked, all tools will be modified.
...
|toolfeature|
	...
	- **Copy to console**, copy the table to the PySisyphe console as a numpy ndarray variable named 'df'.
...
|toolfeature2|
	...
	- **Distance to mesh center of mass**, euclidean distance between the Target/Trajectory and the mesh center of mass.
...
|labels|
	Left-click to locate checked target/trajectory tools in an atlas. A dialog box is displayed in which you can select one or more atlases. The results are similar to those described in :ref:`menu-section-tools-labeling`.
...
Double-click a Target/Trajectory tool to set the cursor position on the target and align the three orthogonal 2D views of the :ref:`page-orthogonalview` to the trajectory tool axe (see the :ref:`section <ortho-section-align>` to select the alignment of the orthogonal view widget).
...
|opacity|
	Left-click to select Target/Trajectory opacity in the view widget(s) (0.0 transparent, 1.0 opaque). Left-click on the slider to increment or decrement the opacity by ten percent. There are also keyboard shortcuts for the slider. The left or up arrow keys decrement the opacity by one percent. The right or down arrow keys increment the opacity by one percent. The PageUp key decrements the opacity by ten percent. The PageDown key increments the opacity by ten percent. The Home key sets the opacity to zero percent (transparent). The End key sets the opacity to one hundred percent (opaque).
...
.. |uncheck| image:: /GUI/dock/uncheck.png
   :scale: 50 %
.. |console| image:: /GUI/dock/console.png
   :scale: 50 %
...
.. |settings| image:: /GUI/dock/settings.png
   :scale: 50 %
.. |labels| image:: /GUI/dock/labels.png
   :scale: 50 %
.. |targets| image:: /GUI/dock/targets.png
   :scale: 50 %