.. _page-synchronizedview:

Synchronized view widget
========================

- :ref:`synchro-section-controls`
- :ref:`synchro-section-iconbar`
- :ref:`synchro-section-popup`

.. image:: /GUI/synchro-view.png
   :align: center
|

This widget, placed in the central area, displays the same slice of multiple synchronized volumes (reference, and all volumes displayed as overlays in others view widgets) in a grid. This widget can be used to display ROI (:ref:`ROI <page-dockroi>`). However, it does not display mesh, trajectory/target tools and streamlines (:ref:`Mesh <page-dockmesh>`, :ref:`Target/Trajectory <page-docktarget>`, :ref:`Tracking <page-docktracking>`).
...
|show|
	...
	- **Line/font properties** (**p** shortkey) to edit line properties (line width in pixels, color and opacity) and font properties (font name, size and scale factor).

.. _synchro-section-info:
...
|info|
	...
	- **ICBM coordinate**, MNI space coordinates at mouse position (if spatial normalization has been performed for the current volume).
...
|iso|
	...
	- |palette| to choose line color. 
	- |opacity2| to choose line opacity.
...
- :ref:`Actions <synchro-section-action>` >
	No action, Move, Zoom, Level/Window, Cursor follows mouse, Cenetred cursor

- :ref:`Visibility <synchro-section-show>` >
...
- :ref:`Information <synchro-section-info>` >
	Identity, Image attributes, Acquisition attribute, Cursor world coordinates, Coordinates relative to AC, Coordinates relative to PC, Coordinates relative to mid AC-PC, Voxel value at mouse position, Orientationmarker shape >
...
- :ref:`Tools <synchro-section-ruler>` > 
	Distance, Orthogonal distance, Angle, Remove all

- :ref:`Save view capture <synchro-section-photo>` >
	Save grid capture..., Save current view capture, Save capture from slice series

- :ref:`Copy view capture to clipboard <synchro-section-clipboard>` > 
	Copy grid capture to clipboard, Copy current view capture to clipboard

The same menu is available via the menu bar ("Views" -> "Synchronized view").
