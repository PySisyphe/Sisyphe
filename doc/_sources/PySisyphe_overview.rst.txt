...
Visualization functions are based on the VTK_ library. Reference libraries provide post-processing: ITK_/SimpleITK_ (filtering, Region-Of-Interest tools), pyradiomics_ (texture analysis), ANTspyx_ (co-registration, prior-based registration, cortical thickness), ANTspynet_ (deep learning segmentation), nilearn_ (time-series analysis), dipy_ (diffusion/tracking analysis).
...
These widgets provide common tools: look-up table management, overlay(s) management, isovalue display, Region-of-Interest (ROI) display, mesh display, target/trajectory tools, measurement tools (distance, orthogonal distances, angle), screenshots management.

The ROI management tools are as follows: various 2D/3D brushes, cut/copy/paste, flip, translations in any direction, interpolating empty slices, automatic or interactive hole filling, mathematical morphology operators, set operators (union/intersection/difference/symmetric difference), thresholding, region-growing segmentation, confidence connected segmentation, active contour (snake) segmentation. Most of these tools can be applied to 2D slices, whole 3D volume or to blob(s) derived from connected component labeling. Any ROI processing can be cancelled with unlimited number of undo/redo.

The interface also includes a patient :ref:`database manager <page-database>`, :ref:`a file manager <page-browser>`, a :ref:`screenshots manager <page-screenshots>` and a fully functional :ref:`IPython console <page-ipython>`.
...
PySisyphe is plug-in extensible and provides a :ref:`fully documented API <page-core-api>` that gives access to all of the software's advanced features and widgets for fast plug-in development. API classes are available in the PySisyphe's :ref:`IPython console <page-ipython>`.