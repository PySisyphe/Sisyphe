...
|open|
	Left-click to show ROI selection dialog (PySisyphe ROI file, .xroi), adds selected ROI(s) to the list and displays them in the view widget(s). **This ROI must be in the same space as the reference volume displayed in the view widget i.e. same space ID**.
...
|console|
	Left-click to copy checked ROI(s) to the PySisyphe console as a list of SisypheROI instances (variable name 'rois').

|view|
	Left-click to show/hide all ROI(s) of the list.
...
|map|
	Left-click to open a popup menu that will calculate and save distance maps of the checked ROI(s). The distance map labels each voxels of the image with the euclidean distance to the nearest boundary of the ROI. Two types of distance maps can be calculated, either with positive values outside the ROI (menu outside positive) or, conversely, with positive values inside the ROI (menu inside positive).

|roi2mesh|
	Left-click to convert checked ROI(s) to mesh(es) and save. 

|texture|
	Left-click to process texture features of checked ROI(s). A dialog box is displayed in which you can select the texture features (`feature definition <https://pyradiomics.readthedocs.io/en/latest/features.html>`_) and kernel radius (in voxels) used by some texture operators.

|labels|
	Left-click to label checked ROI(s) with labels from an atlas. A dialog box is displayed in which you can select one or more atlases. The results are similar to those described in :ref:`menu-section-roi-labeling`.
...
.. |palette| image:: /GUI/dock/palette.png
   :scale: 50 %
.. |texture| image:: /GUI/dock/texture.png
   :scale: 50 %
