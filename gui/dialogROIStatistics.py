...
class DialogROIStatistics(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        # noinspection PyUnresolvedReferences
        self._action['savestats1'].triggered.connect(lambda _, d=True: self._saveStats(default=d))
        # noinspection PyUnresolvedReferences
        self._action['savestats2'].triggered.connect(lambda _, d=False: self._saveStats(default=d))
        # noinspection PyUnresolvedReferences
        self._action['saveallstats1'].triggered.connect(lambda _, d=True: self._saveAllStats(default=d))
        # noinspection PyUnresolvedReferences
        self._action['saveallstats2'].triggered.connect(lambda _, d=False: self._saveAllStats(default=d))
        # noinspection PyUnresolvedReferences
        self._action['savehists1'].triggered.connect(lambda _, d=True: self._saveHistograms(default=d))
        # noinspection PyUnresolvedReferences
        self._action['savehists2'].triggered.connect(lambda _, d=False: self._saveHistograms(default=d))
        ...
        #                                 'Kurtosis'])
        self._graylist.setHeaderLabels(['Name', 'Voxels', 'Volume (cc)', 'Mean', 'Median', 'Min.', 'Max.',
                                        'Range', 'Perc. 25', 'Perc. 75', 'Variance', 'Std Dev.', 'Skewness',
                                        'Kurtosis'])
        # Revision 21/03/2025 >
        ...
        self._shapelist.setHeaderLabels(['Name', 'Volume (cc)', 'Surface', 'Surface/volume\nratio',
                                         'Sphericity', 'Max.\ndiameter', 'Max. axial\ndiameter',
                                         'Max. coronal\ndiameter', 'Max. sagittal\ndiameter', 'Major axis\nlength',
                                         'Minor axis\nlength', 'Least axis\nlength', 'Elongation', 'Flatness'])
        # Revision 21/03/2025 >
        ...
        self._stattab.addTab(self._graytab, 'Grayscale statistics')
        ...
        self._savefig = QPushButton('Save chart')
        ...
        self._copyfig = QPushButton('Copy chart')
        ...
        self._savestats = QPushButton('Save statistics')
        ...
        if self.hasROICollection():
            if len(self._rois) > 0:
                ...
                for roi in self._rois:
                    ...
                    item.setText(1, str(self._stats[name]['count']))
                    item.setText(2, '{:.2f}'.format(self._stats[name]['count'] * v / 1000))
                    item.setText(3, '{:.2f}'.format(self._stats[name]['mean']))
                    item.setText(4, '{:.2f}'.format(self._stats[name]['median']))
                    item.setText(5, '{:.2f}'.format(self._stats[name]['min']))
                    item.setText(6, '{:.2f}'.format(self._stats[name]['max']))
                    item.setText(7, '{:.2f}'.format(self._stats[name]['range']))
                    item.setText(8, '{:.2f}'.format(self._stats[name]['perc25']))
                    item.setText(9, '{:.2f}'.format(self._stats[name]['perc75']))
                    item.setText(10, '{:.2f}'.format(self._stats[name]['var']))
                    item.setText(11, '{:.2f}'.format(self._stats[name]['std']))
                    ...
                    # item.setText(1, '')
                    # item.setBackground(1, QBrush(roi.getQColor()))
                    item.setText(1, '{:.2f}'.format(self._shape[name]['VoxelVolume'] / 1000))
                    item.setText(2, '{:.2f}'.format(self._shape[name]['SurfaceArea']))
                    item.setText(3, '{:.2f}'.format(self._shape[name]['SurfaceVolumeRatio']))
                    item.setText(4, '{:.2f}'.format(self._shape[name]['Sphericity']))
                    item.setToolTip(4, 'Sphericity is a measure of the roundness of the shape of the tumor region\n'
                    ...
                    item.setText(5, '{:.2f}'.format(self._shape[name]['Maximum3DDiameter']))
                    item.setText(6, '{:.2f}'.format(self._shape[name]['Maximum2DDiameterSlice']))
                    item.setText(7, '{:.2f}'.format(self._shape[name]['Maximum2DDiameterColumn']))
                    item.setText(8, '{:.2f}'.format(self._shape[name]['Maximum2DDiameterRow']))
                    item.setText(9, '{:.2f}'.format(self._shape[name]['MajorAxisLength']))
                    item.setText(10, '{:.2f}'.format(self._shape[name]['MinorAxisLength']))
                    item.setText(11, '{:.2f}'.format(self._shape[name]['LeastAxisLength']))
                    item.setText(12, '{:.2f}'.format(self._shape[name]['Elongation']))
                    item.setToolTip(12, 'Elongation shows the relationship between the two largest principal\n'
                    ...
                    item.setText(13, '{:.2f}'.format(self._shape[name]['Flatness']))
                    item.setToolTip(13, 'Flatness shows the relationship between the largest and smallest\n'
        ...
        if filename:
            ...
            try:
                ...
                if default: messageBox(self,
                                       'Save chart',
                                       text='Save chart to {}'.format(filename))
    ...
    def _saveStats(self, index=None, default=False):
        ...
        if index == 0:
            ...
            filename = splitext(self._volume.getFilename())[0] + '_Statistics.csv'
        else:
            title = 'Save shape features'
            filename = splitext(self._volume.getFilename())[0] + '_Shape.csv'
        if not default:
            filename = QFileDialog.getSaveFileName(self, title, filename,
                                                   filter='CSV (*.csv);; '
                                                          'JSON (*.json);; '
                ...
                if len(rois) > 0:
                    ...
                    for roi in rois:
                        if roi.getReferenceID() != self._volume.getID():
                            messageBox(self,
                                       'ROI statistics',
                                       text='ROI {} ID is not same as reference volume.'.format(roi.getName()))
                            continue
                        elif roi.getNumpy().sum() == 0:
                            messageBox(self,
                                       'ROI statistics',
                                       text='ROI {} array is empty.'.format(roi.getName()))
                            continue
