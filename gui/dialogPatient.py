...
class DialogPatient(QDialog):
    ...
    def __init__(self, parent=None, identity=None):
        ...
        if isinstance(identity, SisypheVolume): self._identity = SisypheVolume.getIdentity()
            ...
            for file in files:
                if file != '':
                    ...
                    if splitext(filename)[1] == SisypheVolume.getFileExt():
                        ...
                        except: QMessageBox.warning(self, 'Edit patient', '{} read error.'.format(basename(filename)))
